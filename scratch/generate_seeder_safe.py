...
# Define the complete sections data structure
sections_data = [
    {
...
def escape_php_string(s):
    # Escape backslashes and single quotes for PHP single-quoted string
    return s.replace('\\', '\\\\').replace("'", "\\'")

def format_php_array(d, indent=8):
    if isinstance(d, dict):
        parts = []
        for k, v in d.items():
            k_str = f"'{escape_php_string(k)}'"
            v_str = format_php_array(v, indent + 4)
            parts.append(f"{' ' * indent}{k_str} => {v_str}")
        return "[\n" + ",\n".join(parts) + f",\n{' ' * (indent - 4)}]"
    elif isinstance(d, list):
        parts = []
        for v in d:
            parts.append(format_php_array(v, indent + 4))
        return "[\n" + ",\n".join(f"{' ' * indent}{x}" for x in parts) + f",\n{' ' * (indent - 4)}]"
    elif isinstance(d, str):
        return f"'{escape_php_string(d)}'"
    else:
        return str(d)

php_sections_data = format_php_array(sections_data, 12)

seeder_template = f"""<?php
